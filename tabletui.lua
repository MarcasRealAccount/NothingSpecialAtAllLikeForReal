...
local mobSpawner = {
	...
	getState = function()
		local replyChannel, message = sendMobSpawnerCommand("getState")
	...
	getSettings = function()
		local replyChannel, message = sendMobSpawnerCommand("getSettings")
	...
	setPlayerName = function(newPlayerName)
		local replyChannel, message = sendMobSpawnerCommand(string.format("setPlayerName:%s", newPlayerName))
	...
	setChannel = function(newChannel)
		local replyChannel, message = sendMobSpawnerCommand(string.format("setChannel:%d", newChannel))
		if message == "changed" then
	...
	forceEnable = function()
		local replyChannel, message = sendMobSpawnerCommand("enable")
	...
	forceDisabled = function()
		local replyChannel, message = sendMobSpawnerCommand("disable")
	...
	unforce = function()
		local replyChannel, message = sendMobSpawnerCommand("unforce")
	...
	onDraw = function()
		...
		if mobSpawner.uiState.inSettings then
			...
			if mobSpawner.uiState.selectedTextField == "playername" then
				...
				mobSpawner.drawTextfield(2, mobSpawner.settings.playerName, 1)
			end
			...
			if mobSpawner.uiState.selectedTextField == "channel" then
				...
				mobSpawner.drawTextfield(5, tostring(mobSpawner.settings.listenChannel), 1)
			end
	...
	onEvent = function(event, eventData)
			...
			elseif not mobSpawner.uiState.inSettings then
				...
				if eventData[3] >= 1 and eventData[3] <= 15 then
					if eventData[4] == 2 then
						...
						mobSpawner.uiState.cursorPos         = math.min(eventData[3], #mobSpawner.uiState.curText) - 1
		...
		elseif event == "char" then
			...
			mobSpawner.uiState.curText = mobSpawner.uiState.curText.sub(1, mobSpawner.uiState.cursorPos) .. eventData[2] .. mobSpawner.uiState.curText.sub(1 + mobSpawner.uiState.cursorPos, -1)
			...
			term.setCursorPos(1 + mobSpawner.uiState.cursorPos - mobSpawner.uiState.viewPos, y)
		elseif event == "key" then
			...
			if eventData[2] == keys.backspace then
				mobSpawner.uiState.curText = mobSpawner.uiState.curText.sub(1, mobSpawner.uiState.cursorPos - 1) .. mobSpawner.uiState.curText.sub(1 + mobSpawner.uiState.cursorPos, -1)
			...
			elseif eventData[2] == keys.delete then
				mobSpawner.uiState.curText = mobSpawner.uiState.curText.sub(1, mobSpawner.uiState.cursorPos) .. mobSpawner.uiState.curText.sub(2 + mobSpawner.uiState.cursorPos, -1)
				redrawTextfield = true
...
while true do
	...
	if event == "mouse_click" then
		if eventData[3] >= 20 then
			if eventData[2] >= 1 and eventData[2] <= 5 then
				if progState.tab == "mob" then
					progState.tab = nil
				else
					progState.tab = "mob"
